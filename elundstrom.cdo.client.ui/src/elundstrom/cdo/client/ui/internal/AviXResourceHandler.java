...
public class AviXResourceHandler implements IModelResourceHandler {
	 ...
	 */
	@Inject
	public AviXResourceHandler(@Named(IWorkbench.PERSIST_STATE) boolean saveAndRestore,
	...
	public Resource loadMostRecentModel() {
		...
		if (restore && saveAndRestore) {
			...
			if (!hasTopLevelWindows(resource)) {
				if (logger != null) {
					logger.error(new Exception(), // log a stack trace to help
													// debug the corruption
	...
	protected long getLastApplicationModification() {
		...
		Map<String, ?> attributes = resourceSetImpl.getURIConverter().getAttributes(applicationDefinitionInstance,
